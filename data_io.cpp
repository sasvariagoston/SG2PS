...
vector <vector <GDB> > PROCESS_GROUPS (const vector <vector <GDB> >& inGDB_G) {
	...
	for (size_t i = 0; i < outGDB_G.size(); i++) {
		...
		const bool SC_TO_PROCESS = is_BINGHAM_USE() && IS_SC;
		...
		if (FRACTURE_TO_PROCESS || STRIAE_TO_PROCESS || SC_TO_PROCESS) {
			...
			if (ENOUGH_HOMOGENEOUS && ENOUGH_DATA) {
				...
				if (is_VIRTUAL_USE() && STRIAE_TO_PROCESS) {
					...
					INVERSION (hasoffset_virtual_GDB);
				}
...
void cout_method_text (const vector <GDB>& inGDB) {
	...
	else if (is_BINGHAM_USE() && IS_SC) 				M = "transport direction statistics after Bingham (1964): ";
